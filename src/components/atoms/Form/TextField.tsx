		...
		>
			...
			>
				...
				<input
					...
					id={name}
					className={cn(
						"py-2 text-sm grow h-full bg-transparent outline-none",
			...
			>
				{helperText && !errorText && (
					<p className='text-xs font-medium text-gray-800'>{helperText}</p>
				)}
				{errorText && (
					<p className='text-xs font-medium text-red-error'>{errorText}</p>
				)}
			</div>
		</div>
	);
